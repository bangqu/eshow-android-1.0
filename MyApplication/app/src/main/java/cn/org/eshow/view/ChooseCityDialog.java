...
import cn.org.eshow.adapter.AreaWheelAdapter;
import cn.org.eshow.adapter.CityWheelAdapter;
import cn.org.eshow.adapter.ProviceWheelAdapter;
import cn.org.eshow.bean.AreaBean;
import cn.org.eshow.bean.CityBean;
import cn.org.eshow.bean.ProviceBean;
import cn.org.eshow_structure.util.ESJsonUtil;
import cn.org.eshow_structure.util.ESViewUtil;
import cn.org.eshow_structure.view.wheel.ESWheelView;
...
public class ChooseCityDialog extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        wvProvince = (ESWheelView) findViewById(R.id.wvProvince);
        wvProvince.setValueTextSize(32);
        wvCity = (ESWheelView) findViewById(R.id.wvCity);
        wvCity.setValueTextSize(32);
        wvArea = (ESWheelView) findViewById(R.id.wvArea);
        wvArea.setValueTextSize(32);

        tempCities = new ArrayList<CityBean>();
        tempAreas  = new ArrayList<AreaBean>();

        wvProvince.addChangingListener(new ESWheelView.AbOnWheelChangedListener() {
            @Override
        ...
        wvCity.addChangingListener(new ESWheelView.AbOnWheelChangedListener() {
            @Override
            public void onChanged(ESWheelView wheel, int oldValue, int newValue) {
        ...
        wvArea.addChangingListener(new ESWheelView.AbOnWheelChangedListener() {
            @Override
            public void onChanged(ESWheelView wheel, int oldValue, int newValue) {
    ...
    class DataThread extends Thread {
        ...
        @Override
        public void run() {
            provices = (List<ProviceBean>) ESJsonUtil.fromJson(getJson(context, proviceJsonFile), new TypeToken<ArrayList<ProviceBean>>() {
            });
            cities = (List<CityBean>) ESJsonUtil.fromJson(getJson(context, cityJsonFile), new TypeToken<ArrayList<CityBean>>() {
            });
            areas = (List<AreaBean>) ESJsonUtil.fromJson(getJson(context, areaJsonFile), new TypeToken<ArrayList<AreaBean>>() {
            });
