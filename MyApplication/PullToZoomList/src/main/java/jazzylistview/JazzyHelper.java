...
import jazzylistview.effects.CardsEffect;
import jazzylistview.effects.CurlEffect;
import jazzylistview.effects.FadeEffect;
import jazzylistview.effects.FanEffect;
import jazzylistview.effects.FlipEffect;
import jazzylistview.effects.FlyEffect;
import jazzylistview.effects.GrowEffect;
import jazzylistview.effects.HelixEffect;
import jazzylistview.effects.ReverseFlyEffect;
import jazzylistview.effects.SlideInEffect;
import jazzylistview.effects.StandardEffect;
import jazzylistview.effects.TiltEffect;
import jazzylistview.effects.TwirlEffect;
import jazzylistview.effects.WaveEffect;
import jazzylistview.effects.ZipperEffect;
...
public class JazzyHelper implements AbsListView.OnScrollListener {
    ...
    public void onStartAnimate(int xPoint){
        if(view != null && firstVisibleItem >= 0 && visibleItemCount > 0 && totalItemCount >0){
            ...
            for(int i = firstVisibleItem; i < visibleItemCount; i++){
                ...
                doJazzinessImpl(item, i,xPoint, 1,delay,DURATION);
            }
        }
    }
    /**
     ...
     * @param duration
     */
    private void doJazzinessImpl(View item, int position,int xPoint, int scrollDirection,int delay,int duration) {
        if (item == null) {
            return;
        }
        ...
        scrollDirection = scrollDirection > 0 ? 1 : -1;
        WaveEffect waveEffect = (WaveEffect) mTransitionEffect;
        waveEffect.initView(item,xPoint);
        waveEffect.setupAnimation(item, position, scrollDirection, animator);
        animator.start();
    }
