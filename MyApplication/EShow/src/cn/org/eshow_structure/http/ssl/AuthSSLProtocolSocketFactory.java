package cn.org.eshow_structure.http.ssl;
...
import cn.org.eshow_structure.http.ESHttpClient;
...
public class AuthSSLProtocolSocketFactory implements LayeredSocketFactory {
	...
	private static KeyStore createKeyStore(final URL url, final String password)
		...
		ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "Initializing key store");
	...
	private static KeyManager[] createKeyManagers(final KeyStore keystore,
		...
		ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "Initializing key manager");
	...
	private static TrustManager[] createTrustManagers(final KeyStore keystore)
		...
		ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "Initializing trust manager");
		...
		try {
			...
			if (this.keystoreUrl != null) {
				...
				if (ESLogUtil.D) {
					...
					while (aliases.hasMoreElements()) {
						...
						if (certs != null) {
							ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "Certificate chain '" + alias + "':");
							for (int c = 0; c < certs.length; c++) {
								if (certs[c] instanceof X509Certificate) {
									X509Certificate cert = (X509Certificate) certs[c];
									ESLogUtil.d(AuthSSLProtocolSocketFactory.class, " Certificate " + (c + 1) + ":");
									ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Subject DN: "
											+ cert.getSubjectDN());
									ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Signature Algorithm: "
											+ cert.getSigAlgName());
									ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Valid from: "
											+ cert.getNotBefore());
									ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Valid until: "
											+ cert.getNotAfter());
									ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Issuer: " + cert.getIssuerDN());
								}
							}
						}
					}
			...
			if (this.truststoreUrl != null) {
				...
				if (ESLogUtil.D) {
					...
					while (aliases.hasMoreElements()) {
						String alias = (String) aliases.nextElement();
						ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "Trusted certificate '" + alias + "':");
						Certificate trustedcert = keystore
								.getCertificate(alias);
						if (trustedcert != null
								&& trustedcert instanceof X509Certificate) {
							X509Certificate cert = (X509Certificate) trustedcert;
							ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Subject DN: " + cert.getSubjectDN());
							ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Signature Algorithm: "
									+ cert.getSigAlgName());
							ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Valid from: " + cert.getNotBefore());
							ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Valid until: " + cert.getNotAfter());
							ESLogUtil.d(AuthSSLProtocolSocketFactory.class, "  Issuer: " + cert.getIssuerDN());
						}
					}
				}
			...
			ESLogUtil.e(AuthSSLProtocolSocketFactory.class, e.getMessage());
	...
	public Socket createSocket(final String host, final int port,
		...
		int timeout = ESHttpClient.DEFAULT_SOCKET_TIMEOUT;
