 ...
 */
package cn.org.eshow_structure.db.orm.dao;
...
import cn.org.eshow_structure.db.ESBasicDBDao;
import cn.org.eshow_structure.db.orm.ESTableHelper;
import cn.org.eshow_structure.db.orm.annotation.ActionType;
import cn.org.eshow_structure.db.orm.annotation.Column;
import cn.org.eshow_structure.db.orm.annotation.Id;
import cn.org.eshow_structure.db.orm.annotation.Relations;
import cn.org.eshow_structure.db.orm.annotation.RelationsType;
import cn.org.eshow_structure.db.orm.annotation.Table;
 ...
 *
 */
public class ESDBDaoImpl<T> extends ESBasicDBDao implements ESDBDao<T> {
	 ...
	 * @param clazz 映射对象实体
	 */
	public ESDBDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		// 加载所有字段
		this.allFields = ESTableHelper.joinFields(this.clazz.getDeclaredFields(),
		...
		ESLogUtil.d(ESDBDaoImpl.class, "clazz:" + this.clazz + " tableName:" + this.tableName
	 ...
	 * @param dbHelper 数据库操作实现类
	 */
	public ESDBDaoImpl(SQLiteOpenHelper dbHelper) {
		this(dbHelper, null);
	}
	...
	public T queryOne(int id) {
		synchronized (lock) {
			...
			ESLogUtil.d(ESDBDaoImpl.class, "[queryOne]: select * from " + this.tableName + " where "
					+ this.idColumn + " = '" + id + "'");
	 ...
	 * @param clazz  返回的对象类型
	 * @return the list
	 * @see ESDBDao#rawQuery(java.lang.String, java.lang.String[])
	 */
	@Override
	public List<T> rawQuery(String sql, String[] selectionArgs,Class<T> clazz) {
		...
		try {
			lock.lock();
			checkDBOpened();
			ESLogUtil.d(ESDBDaoImpl.class, "[rawQuery]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			getListFromCursor(clazz,list, cursor);
		} catch (Exception e) {
			ESLogUtil.e(ESDBDaoImpl.class, "[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	 ...
	 * @see ESDBDao#isExist(java.lang.String, java.lang.String[])
	 */
	@Override
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			ESLogUtil.d(ESDBDaoImpl.class, "[isExist]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			if (cursor.getCount() > 0) {
				return true;
			}
		} catch (Exception e) {
			ESLogUtil.e(ESDBDaoImpl.class, "[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	 ...
	 * @return the list
	 * @see ESDBDao#queryList(java.lang.String[], java.lang.String, java.lang.String[], java.lang.String, java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
	public List<T> queryList(String[] columns, String selection,
			...
			try {
				lock.lock();
				checkDBOpened();
				ESLogUtil.d(ESDBDaoImpl.class, "[queryList] from " + this.tableName + " where " + selection + "(" + selectionArgs + ")" + " group by " + groupBy + " having " + having + " order by " + orderBy + " limit " + limit);
					...
					for(T entity:list){
							...
							if(RelationsType.one2one.equals(type)){
								...
								if(listEntityClazz==null){
									ESLogUtil.e(ESDBDaoImpl.class, "对象模型需要设置List的泛型");
									return null;
								}
				...
				ESLogUtil.e(ESDBDaoImpl.class, "[queryList] from DB Exception");
				e.printStackTrace();
			} finally {
				closeCursor(cursor);
				lock.unlock();
     ...
     * @param selectionArgs the selection args
     * @return the list
     * @see ESDBDao#queryList(java.lang.String, java.lang.String[])
     * @author: amsoft.cn
     */
	@Override
	...
	private void getListFromCursor(Class<?> clazz,List<T> list, Cursor cursor)
		...
		while (cursor.moveToNext()) {
			...
			// 加载所有字段
			List<Field> allFields = ESTableHelper.joinFields(entity.getClass().getDeclaredFields(),
	 ...
	 * @return the long
	 * @see ESDBDao#insert(java.lang.Object)
	 */
	@Override
	 ...
	 * @param flag the flag
	 * @return the long
	 * @see ESDBDao#insert(java.lang.Object, boolean)
	 */
	@Override
	public long insert(T entity, boolean flag) {
			...
			try {
				...
				ESLogUtil.d(ESDBDaoImpl.class, "[insert]: insert into " + this.tableName + " " + sql);
				...
				for (Field relationsField : allFields) {
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							ESLogUtil.d(ESDBDaoImpl.class, "[insert]: insert into " + relationsTableName + " " + sql);
				...
				ESLogUtil.d(ESDBDaoImpl.class, "[insert] into DB Exception.");
				e.printStackTrace();
			}finally {
				lock.unlock();
			}
	 ...
	 * @see ESDBDao#insertList(java.util.List)
	 */
	@Override
	 ...
	 * @param flag the flag
	 * @return the long[] 插入成功的数据ID
	 * @see ESDBDao#insertList(java.util.List, boolean)
	 */
	@Override
	public long[] insertList(List<T> entityList,boolean flag) {
			...
			try {
				...
				for(int i=0;i<entityList.size();i++){
					...
					ESLogUtil.d(ESDBDaoImpl.class, "[insertList]: insert into " + this.tableName + " " + sql);
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							ESLogUtil.d(ESDBDaoImpl.class, "[insertList]: insert into " + relationsTableName + " " + sql);
							db.insert(relationsTableName, null, relationsCv);
						}
				...
				ESLogUtil.d(ESDBDaoImpl.class, "[insertList] into DB Exception.");
				e.printStackTrace();
			} finally {
				lock.unlock();
			}
	...
	public int delete(int id) {
		...
		try {
			...
			ESLogUtil.d(ESDBDaoImpl.class, "[delete]: delelte from " + this.tableName + " where "
					+ where.replace("?", String.valueOf(id)));
			rows =  db.delete(this.tableName, where, whereValue);
		} catch (Exception e) {
			ESLogUtil.d(ESDBDaoImpl.class, "[delete] DB Exception.");
			e.printStackTrace();
		}finally{
			lock.unlock();
	    }
	 ...
	 * @param ids the ids
	 * @return the int
	 * @see ESDBDao#delete(java.lang.Integer[])
	 */
	@Override
	 ...
	 * @return the int
	 * @see ESDBDao#delete(java.lang.String, java.lang.String[])
	 */
	@Override
	public int delete(String whereClause, String[] whereArgs) {
		...
		try {
			...
			if(!ESStrUtil.isEmpty(mLogSql)){
				mLogSql +=" where ";
			}
			ESLogUtil.d(ESDBDaoImpl.class, "[delete]: delete from " + this.tableName + mLogSql);
	...
	public int deleteAll() {
		...
		try {
			lock.lock();
			checkDBOpened();
			ESLogUtil.d(ESDBDaoImpl.class, "[delete]: delete from " + this.tableName);
	 ...
	 * @param entity the entity
	 * @return the int 影响的行
	 * @see ESDBDao#update(java.lang.Object)
	 */
	@Override
	public int update(T entity) {
		...
		try {
			...
			ESLogUtil.d(ESDBDaoImpl.class, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", String.valueOf(id)));

			String[] whereValue = { Integer.toString(id) };
			rows = db.update(this.tableName, cv, where, whereValue);
		} catch (Exception e) {
			ESLogUtil.d(ESDBDaoImpl.class, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
	 ...
	 * @param entityList the entity list
	 * @return the int 影响的行
	 * @see ESDBDao#updateList(java.util.List)
	 */
	@Override
	 ...
	 * @param selectionArgs the selection args
	 * @return the list
	 * @see ESDBDao#queryMapList(java.lang.String, java.lang.String[])
	 */
	@Override
	public List<Map<String, String>> queryMapList(String sql,String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			ESLogUtil.d(ESDBDaoImpl.class, "[queryMapList]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			while (cursor.moveToNext()) {
			...
			ESLogUtil.e(ESDBDaoImpl.class, "[queryMapList] from DB exception");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	 ...
	 * @param selectionArgs the selection args
	 * @return the int
	 * @see ESDBDao#queryCount(java.lang.String, java.lang.String[])
	 */
	@Override
	public int queryCount(String sql, String[] selectionArgs) {
       ...
       try{
    	   lock.lock();
    	   checkDBOpened();
    	   ESLogUtil.d(ESDBDaoImpl.class, "[queryCount]: " + getLogSql(sql, selectionArgs));
           cursor = db.query(this.tableName, null, sql, selectionArgs, null, null,null);
           if(cursor != null){
        	   count = cursor.getCount();
           }
       }catch (Exception e){
    	   ESLogUtil.e(ESDBDaoImpl.class, "[queryCount] from DB exception");
           e.printStackTrace();
       }finally{
    	   closeCursor(cursor);
    	   lock.unlock();
	 ...
	 * @param sql the sql
	 * @param selectionArgs the selection args
	 * @see ESDBDao#execSql(java.lang.String, java.lang.Object[])
	 */
	@Override
	public void execSql(String sql, Object[] selectionArgs) {
		try {
			lock.lock();
			checkDBOpened();
			ESLogUtil.d(ESDBDaoImpl.class, "[execSql]: " + getLogSql(sql, selectionArgs));
			...
			ESLogUtil.e(ESDBDaoImpl.class, "[execSql] DB exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
