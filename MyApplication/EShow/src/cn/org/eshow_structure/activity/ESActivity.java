 ...
 */
package cn.org.eshow_structure.activity;
...
import cn.org.eshow_structure.view.ioc.ESIocEventListener;
import cn.org.eshow_structure.view.ioc.ESIocSelect;
import cn.org.eshow_structure.view.ioc.ESIocView;
import cn.org.eshow_structure.view.titlebar.ESBottomBar;
import cn.org.eshow_structure.view.titlebar.ESTitleBar;

import java.lang.reflect.Field;

// TODO: Auto-generated Javadoc
/**
 * 继承这个Activity使你的Activity拥有一个程序框架.
 */
...
public abstract class ESActivity extends FragmentActivity {
	...
	/** 标题栏布局. */
	private ESTitleBar mAbTitleBar = null;
	
	/** 副标题栏布局. */
	private ESBottomBar mAbBottomBar = null;
	 ...
	 */
	public ESBottomBar getBottomBar() {
		return mAbBottomBar;
	}
		...
		if(fields!=null && fields.length>0){
			for(Field field : fields){
				try {
					...
					ESIocView viewInject = field.getAnnotation(ESIocView.class);
					if(viewInject!=null){
						
						int viewId = viewInject.id();
					    field.set(this,findViewById(viewId));
					
						setListener(field,viewInject.click(), ESIocEventListener.CLICK);
						setListener(field,viewInject.longClick(), ESIocEventListener.LONGCLICK);
						setListener(field,viewInject.itemClick(), ESIocEventListener.ITEMCLICK);
						setListener(field,viewInject.itemLongClick(), ESIocEventListener.ITEMLONGCLICK);
						
						ESIocSelect select = viewInject.select();
	...
	private void setViewSelectListener(Field field,String select,String noSelect)throws Exception{
		...
		if(obj instanceof View){
			((AbsListView)obj).setOnItemSelectedListener(new ESIocEventListener(this).select(select).noSelect(noSelect));
		}
	}
	...
	private void setListener(Field field,String methodName,int method)throws Exception{
		...
		switch (method) {
			case ESIocEventListener.CLICK:
				if(obj instanceof View){
					((View)obj).setOnClickListener(new ESIocEventListener(this).click(methodName));
				}
				break;
			case ESIocEventListener.ITEMCLICK:
				if(obj instanceof AbsListView){
					((AbsListView)obj).setOnItemClickListener(new ESIocEventListener(this).itemClick(methodName));
				}
				break;
			case ESIocEventListener.LONGCLICK:
				if(obj instanceof View){
					((View)obj).setOnLongClickListener(new ESIocEventListener(this).longClick(methodName));
				}
				break;
			case ESIocEventListener.ITEMLONGCLICK:
				if(obj instanceof AbsListView){
					((AbsListView)obj).setOnItemLongClickListener(new ESIocEventListener(this).itemLongClick(methodName));
				}
				break;
			default:
				break;
