...
import cn.org.eshow_structure.util.dct.FDCT;
...
public class ESImageUtil {
	...
	public static Bitmap getBitmap(InputStream inputStream,int desiredWidth, int desiredHeight) {
		Bitmap bitmap = null;
		try {
			byte [] data = ESStreamUtil.stream2bytes(inputStream);
	...
	private static boolean checkBitmap(Bitmap bitmap) {
		if (bitmap == null) {
			ESLogUtil.e(ESImageUtil.class, "原图Bitmap为空了");
			return false;
		}

		if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
			ESLogUtil.e(ESImageUtil.class, "原图Bitmap大小为0");
			return false;
		}
		return true;
	}
	...
	private static boolean checkSize(int desiredWidth, int desiredHeight) {
		if (desiredWidth <= 0 || desiredHeight <= 0) {
			ESLogUtil.e(ESImageUtil.class, "请求Bitmap的宽高参数必须大于0");
			return false;
		}
		return true;
	}
	...
	public static byte[] view2Bytes(View view,
		...
		byte[] b = null;
		try {
			Bitmap bitmap = ESImageUtil.view2Bitmap(view);
			b = ESImageUtil.bitmap2Bytes(bitmap, compressFormat, true);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return b;
	...
	public static String getHashCode(Bitmap bitmap) {
		...
		// 第三步，计算平均值。
		// 计算所有64个像素的灰度平均值。
		int avgPixel = ESMathUtil.average(pixels);
		...
		for (int i = 0; i < comps.length; i += 4) {
			...
			hashCode.append(ESMathUtil.binaryToHex(result));
		}
	...
	public static String getDCTHashCode(Bitmap bitmap) {
		...
		int[][] pxMatrix = ESMathUtil.arrayToMatrix(pixels, width, height);
		double[][] doublePxMatrix = ESMathUtil.intToDoubleMatrix(pxMatrix);

		// 计算DCT,已经变成8*8了
		double[][] dtc = FDCT.fDctTransform(doublePxMatrix);

		// 计算平均值。
		double[] dctResult = ESMathUtil.matrixToArray(dtc);
		int avgPixel = ESMathUtil.average(dctResult);
