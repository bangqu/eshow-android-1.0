...
import cn.org.eshow_structure.view.app.ESMonitorView;
 ...
 *
 */
public final class ESMonitorUtil {
	
	/** 性能测试开关状态. */
	public static boolean mMonitorOpened = false;
	
	/** 帧测试View. */
	private static ESMonitorView mAbMonitorView  = null;
	...
	public static void openMonitor(Context context){
		...
		DisplayMetrics display = ESAppUtil.getDisplayMetrics(context);
		final int diaplayWidth = display.widthPixels;
		
		mAbMonitorView  = new ESMonitorView(context);
         ...
         */
        // 设置悬浮窗的长得宽
		mMonitorParams.width = ESViewUtil.scaleValue(context, 100);
		mMonitorParams.height = ESViewUtil.scaleValue(context, 50);
