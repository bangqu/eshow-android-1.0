...
import cn.org.eshow_structure.view.pullview.ESListViewFooter;
import cn.org.eshow_structure.view.pullview.ESListViewHeader;
...
public class ESViewUtil {
	...
	public static int scaleValue(Context context, float value) {
		...
		if(mDisplayMetrics.scaledDensity > ESAppConfig.UI_DENSITY){
			//密度
			if(width > ESAppConfig.UI_WIDTH){
				value = value*(1.3f - 1.0f/mDisplayMetrics.scaledDensity);
			}else if(width < ESAppConfig.UI_WIDTH){
	...
	public static int scale(int displayWidth, int displayHeight, float pxValue) {
		...
		try {
			...
			float scaleWidth = (float) width / ESAppConfig.UI_WIDTH;
			float scaleHeight = (float) height / ESAppConfig.UI_HEIGHT;
    ...
    public static void scaleContentView(ViewGroup contentView){
    	ESViewUtil.scaleView(contentView);
    ...
    public static boolean isNeedScale(View view){
    	if (view instanceof ESListViewHeader){
    		return false;
        }
    	
    	if (view instanceof ESListViewFooter){
    		return false;
        }
    	return true;
    }
    ...
    */
    public static void setViewSize(View view,int widthPixels, int heightPixels){
        ...
        if(params == null){
            ESLogUtil.e(ESViewUtil.class, "setViewSize出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
