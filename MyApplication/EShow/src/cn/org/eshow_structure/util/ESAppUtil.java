...
import cn.org.eshow_structure.model.ESAppProcessInfo;
import cn.org.eshow_structure.model.ESCPUInfo;
import cn.org.eshow_structure.model.ESProcessInfo;
import cn.org.eshow_structure.model.ESPsRow;
...
public class ESAppUtil {
     ...
     * @return
     */
    public static List<ESAppProcessInfo> getRunningAppProcesses(Context context) {
    	ActivityManager activityManager = null;
    	List<ESAppProcessInfo> list = null;
    	PackageManager packageManager = null;
	    try {
	    	activityManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
	    	packageManager = context.getApplicationContext().getPackageManager();
	    	list = new ArrayList<ESAppProcessInfo>();
	    	//所有运行的进程
	    	List<RunningAppProcessInfo> appProcessList = activityManager.getRunningAppProcesses();
	    	ApplicationInfo appInfo = null;
	    	ESAppProcessInfo abAppProcessInfo = null;
	    	...
	    	for(RunningAppProcessInfo appProcessInfo:appProcessList){
	    		abAppProcessInfo = new ESAppProcessInfo(appProcessInfo.processName,appProcessInfo.pid,appProcessInfo.uid);
	    		...
	    		ESProcessInfo processInfo = getMemInfo(appProcessInfo.processName);
    ...
    public static List<ESPsRow> ps() {
    	List<ESPsRow> psRowlist = new ArrayList<ESPsRow>();
        String ps = runScript("ps");
        String[] lines = ps.split("\n");
        psRowlist = new ArrayList<ESPsRow>();
        for (String line : lines) {
        	ESPsRow row = new ESPsRow(line);
     ...
     * @return
     */
    public static ESPsRow getPsRow(String processName) {
    	List<ESPsRow> psRowlist = ps();
        for (ESPsRow row : psRowlist) {
     ...
     * @return
     */
    public static ESProcessInfo getMemInfo(String processName) {
    	ESProcessInfo process = new ESProcessInfo();
    	if(mProcessList == null){
    		mProcessList = getProcessRunningInfo();
    	}
        ...
        if(process.memory == 0){
        	ESLogUtil.d(ESAppUtil.class, "##" + processName + ",top -n 1未找到");
        }
        return process;
    }
     ...
     * @return
     */
    public static ESProcessInfo getMemInfo(int pid) {
    	ESProcessInfo process = new ESProcessInfo();
    	if(mProcessList == null){
    		mProcessList = getProcessRunningInfo();
    	}
	...
	public static String runCommand(String[] command, String workdirectory){
		String result = "";
		ESLogUtil.d(ESAppUtil.class, "#" + command);
		try {
	 ...
	 * @return
	 */
	public static ESCPUInfo parseCPUInfo(String info) {
		 ESCPUInfo CPUInfo = new ESCPUInfo();
		 String tempString = "";
		 String[] rows = null;
		 String[] columns = null;
		 rows = info.split("[\n]+");
	...
	public static long getTotalMemory(Context context){  
        ...
        try{  
            ...
            for(String str:strs){  
                ESLogUtil.d(ESAppUtil.class, str + "\t");
            }  
            //获得系统总内存，单位KB  
