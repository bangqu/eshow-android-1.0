...
public class ESFileUtil {
	...
	private static int freeSdSpaceNeededToCache = 200*1024*1024;
	 ...
	 public static Bitmap getBitmapFromSD(File file,int type,int desiredWidth, int desiredHeight){
		 ...
		 try {
			 ...
			 //文件存在
			 if(type == ESImageUtil.CUTIMG){
				bitmap = ESImageUtil.getCutBitmap(file, desiredWidth, desiredHeight);
		 	 }else if(type == ESImageUtil.SCALEIMG){
		 		bitmap = ESImageUtil.getScaleBitmap(file, desiredWidth, desiredHeight);
		 	 }else{
		 		bitmap = ESImageUtil.getBitmap(file);
		 	 }
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Bitmap getBitmapFromURL(String url,int desiredWidth, int desiredHeight){
		Bitmap bitmap = null;
		try {
			bitmap = ESImageUtil.getBitmap(url, desiredWidth, desiredHeight);
	    } catch (Exception e) {
	    	ESLogUtil.d(ESFileUtil.class, "下载图片异常：" + e.getMessage());
		}
		return bitmap;
	}
	...
	public static Bitmap getBitmapFromSrc(String src){
		Bitmap bit = null;
		try {
			bit = BitmapFactory.decodeStream(ESFileUtil.class.getResourceAsStream(src));
	    } catch (Exception e) {
	    	ESLogUtil.d(ESFileUtil.class, "获取图片异常：" + e.getMessage());
		}
		return bit;
	}
 	 ...
 	 public static String downloadFile(String url,String dirPath){
 		 ...
 		 try {
            ...
            if(!file.exists()){
 			...
 			e.printStackTrace();
 			ESLogUtil.e(ESFileUtil.class, "有文件下载出错了,已删除");
	...
	public static int getContentLengthFromUrl(String Url){
		...
		try {
	    	 ...
	    	 e.printStackTrace();
	    	 ESLogUtil.d(ESFileUtil.class, "获取长度异常：" + e.getMessage());
		}
    ...
    public static String getRealFileNameFromUrl(String url){
        ...
        try {
            ...
            e.printStackTrace();
            ESLogUtil.e(ESFileUtil.class, "网络上获取文件名失败");
        }
        return name;
    }
    ...
    public static String getCacheFileNameFromUrl(String url,HttpResponse response){
        ...
        try {
            ...
            if(ESStrUtil.isEmpty(suffix)){
                suffix = ".ab";
            }
            name = ESMd5.MD5(url)+suffix;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return name;
   ...
   public static void copyAssets2SD(Context context, String assetDir, String outDir) {
	...
	public static void initFileDir(Context context){
		...
		//默认下载文件根目录. 
		String downloadRootPath = File.separator + ESAppConfig.DOWNLOAD_ROOT_DIR + File.separator + info.packageName + File.separator;
		
	    //默认下载图片文件目录. 
		String imageDownloadPath = downloadRootPath + ESAppConfig.DOWNLOAD_IMAGE_DIR + File.separator;
	    
	    //默认下载文件目录.
		String fileDownloadPath = downloadRootPath + ESAppConfig.DOWNLOAD_FILE_DIR + File.separator;
		
		//默认缓存目录.
		String cacheDownloadPath = downloadRootPath + ESAppConfig.CACHE_DIR + File.separator;
		
		//默认DB目录.
		String dbDownloadPath = downloadRootPath + ESAppConfig.DB_DIR + File.separator;
