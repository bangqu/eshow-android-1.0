 ...
 *
 */
public class ESListViewHeader extends LinearLayout {
	 ...
	 */
	public ESListViewHeader(Context context) {
		super(context);
		initView(context);
	}
	 ...
	 */
	public ESListViewHeader(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView(context);
	}
	...
	private void initView(Context context) {
		...
		ESViewUtil.setPadding(headerView, 0, 10, 0, 10);
		
		//显示箭头与进度
		FrameLayout headImage =  new FrameLayout(context);
		arrowImageView = new ImageView(context);
		//从包里获取的箭头图片
		arrowImage = ESFileUtil.getBitmapFromSrc("image/arrow.png");
		...
		layoutParamsWW.width = ESViewUtil.scaleValue(mContext, 50);
		layoutParamsWW.height = ESViewUtil.scaleValue(mContext, 50);
		...
		ESViewUtil.setPadding(headTextLayout, 0, 0, 0, 0);
		...
		ESViewUtil.setTextSize(tipsTextview, 30);
		ESViewUtil.setTextSize(headerTimeView, 27);
		...
		layoutParamsWW3.rightMargin = ESViewUtil.scaleValue(mContext, 10);
		...
		switch(state){
			case STATE_NORMAL:
				...
				if(lastRefreshTime==null){
					lastRefreshTime = ESDateUtil.getCurrentDate(ESDateUtil.dateFormatHMS);
