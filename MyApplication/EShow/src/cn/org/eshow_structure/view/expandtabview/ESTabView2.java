package cn.org.eshow_structure.view.expandtabview;
...
import cn.org.eshow_structure.model.ESMenuItem;

public class ESTabView2 extends LinearLayout{
	
	private ListView listView1;
	private ListView listView2;
	
	private ESTabListAdapter listViewAdapter1;
	private ESTabListAdapter listViewAdapter2;
	
	private List<ESMenuItem> groups;
	private List<ArrayList<ESMenuItem>> childrens;
	
	/**当前显示的分类2*/
	private List<ESMenuItem> childrenItem = new ArrayList<ESMenuItem>();
	...
	public ESTabView2(Context context, List<ESMenuItem> groups, List<ArrayList<ESMenuItem>> childrens, int defaultGroupId, int defaultChildrenId, int selectGroupResId, int selectorGroupResId, int selectChildrenResId, int selectorChildrenResId) {
		super(context);
		this.groups = groups;
	...
	public ESTabView2(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void init(Context context) {
		...
		listViewAdapter1 = new ESTabListAdapter(context, groups,selectGroupResId,selectorGroupResId);
		...
		listViewAdapter1.setOnItemClickListener(new ESTabListAdapter.OnItemClickListener() {
			@Override
	    ...
	    listViewAdapter2 = new ESTabListAdapter(context, childrenItem,selectChildrenResId,selectorChildrenResId);
	    listViewAdapter2.setSelectedPositionNoNotify(defaultChildrenPosition);
	    listView2.setAdapter(listViewAdapter2);
	    listViewAdapter2.setOnItemClickListener(new ESTabListAdapter.OnItemClickListener() {
