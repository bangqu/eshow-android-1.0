 ...
 */
package cn.org.eshow_structure.view.chart;
...
import cn.org.eshow_structure.util.ESGraphicUtil;
...
public class GraphicalView extends View {
   ...
   */
  public GraphicalView(Context context, ESstractChart chart) {
    super(context);
    mContext = context;
    mChart = chart;
    ...
    if (mRenderer.isZoomButtonsVisible()) {
      zoomInImage = ESFileUtil.getBitmapFromSrc("image/zoom_in.png");
      zoomOutImage = ESFileUtil.getBitmapFromSrc("image/zoom_out.png");
      fitZoomImage = ESFileUtil.getBitmapFromSrc("image/zoom-1.png");
    }
    ...
    if (mChart instanceof XYChart) {
        ...
        //按分辨率转换
        mXYMultipleSeriesRenderer.setExplainTextSize1(ESViewUtil.scaleTextValue(mContext, explainTextSize1));
        mXYMultipleSeriesRenderer.setExplainTextSize2(ESViewUtil.scaleTextValue(mContext, explainTextSize2));
        mXYMultipleSeriesRenderer.setScaleCircleRadius(ESViewUtil.scaleValue(mContext, scaleCircleRadius));
        mXYMultipleSeriesRenderer.setScaleRectWidth(ESViewUtil.scaleValue(mContext, scaleRectWidth));
        mXYMultipleSeriesRenderer.setScaleRectHeight(ESViewUtil.scaleValue(mContext, scaleRectHeight));
        ...
        if(mSimpleSeriesRenderers!=null && mSimpleSeriesRenderers.length>0){
        	for(int i=0;i<mSimpleSeriesRenderers.length;i++){
        		...
        		mSimpleSeriesRenderer.setChartValuesTextSize(ESViewUtil.scaleTextValue(mContext, mChartValuesTextSize));
        	}
        }
    }
    
    int chartTitleTextSize = (int)mRenderer.getChartTitleTextSize();
    mRenderer.setChartTitleTextSize(ESViewUtil.scaleTextValue(mContext, chartTitleTextSize));
    //轴线上标签文字大小
    int mLabelsTextSize  = (int)mRenderer.getLabelsTextSize();
    mRenderer.setLabelsTextSize(ESViewUtil.scaleTextValue(mContext, mLabelsTextSize));
  	//说明文字大小
    int mLegendTextSize  =  (int)mRenderer.getLegendTextSize();
    mRenderer.setLegendTextSize(ESViewUtil.scaleTextValue(mContext, mLegendTextSize));
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mChart instanceof XYChart) {
	    ...
	    if(mXYMultipleSeriesRenderer.isScaleLineEnabled()){
	        ...
	        int scaleCircleRadius = mXYMultipleSeriesRenderer.getScaleCircleRadius();
	        //按分辨率转换
	        scaleTopPadding = ESViewUtil.scaleValue(mContext, scaleTopPadding);
	        scaleBottomPadding = ESViewUtil.scaleValue(mContext, scaleBottomPadding);
	        ...
	        if(mChart instanceof XYChart){
	          ...
	          if(minValue<5){
	              ...
	              if(showRect){
	                  ...
	                  //行高
	                  float hSize1 = ESGraphicUtil.getDesiredHeight(mTextPaint1);
	                  
	                  //行高
	                  float hSize2 = ESGraphicUtil.getDesiredHeight(mTextPaint2);
	                  
	                  int textPadding = 8;
	                  textPadding = ESViewUtil.scaleValue(mContext, textPadding);
	                  
	                  //设置个新的长方形  
	                  //判断文字是否超出设置的框框
	                  int row1 = ESGraphicUtil.getDrawRowCount(showValue, mXYMultipleSeriesRenderer.getScaleRectWidth() - 10, mTextPaint1);
	                  int row2 = ESGraphicUtil.getDrawRowCount(showExplain, mXYMultipleSeriesRenderer.getScaleRectWidth() - 10, mTextPaint2);
	                  RectF mRectF = null;
	                  ...
	                  ESGraphicUtil.drawText(canvas, showValue, mXYMultipleSeriesRenderer.getScaleRectWidth(), mTextPaint1, mRectLeft + textPadding, mRectTop + textPadding);
	                  ESGraphicUtil.drawText(canvas, showExplain, mXYMultipleSeriesRenderer.getScaleRectWidth(), mTextPaint2, mRectLeft + textPadding, (int) (mRectTop + textPadding + row1 * hSize1));
