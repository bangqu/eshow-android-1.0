...
import cn.org.eshow_structure.adapter.ESFragmentPagerAdapter;
import cn.org.eshow_structure.util.ESLogUtil;
import cn.org.eshow_structure.util.ESViewUtil;
import cn.org.eshow_structure.view.sample.ESHorizontalScrollView;
import cn.org.eshow_structure.view.sample.ESHorizontalScrollView.AbOnScrollListener;
 ...
 *
 */
public class ESSlidingSmoothTabView extends LinearLayout {
	...
	/** tab的线性布局父. */
	private ESHorizontalScrollView mTabScrollView  = null;
	...
	public ESSlidingSmoothTabView(Context context, AttributeSet attrs) {
		...
		if(!(this.context instanceof FragmentActivity)){
			ESLogUtil.e(ESSlidingSmoothTabView.class, "构造AbSlidingSmoothTabView的参数context,必须是FragmentActivity的实例。");
		}
		
		FragmentManager mFragmentManager = ((FragmentActivity)this.context).getFragmentManager();
		mFragmentPagerAdapter = new ESFragmentPagerAdapter(
		...
		mTabScrollView.setOnScrollListener(new AbOnScrollListener() {
			...
			public void onScroll(int arg1) {
				...
				ESLogUtil.d(ESSlidingSmoothTabView.class, "滑动X" + startX + "to" + toX);
	...
	public void computeTabImg(int index){
		...
		ESLogUtil.d(ESSlidingSmoothTabView.class, "view宽度" + index + ":" + tabView.getMeasuredWidth());
		ESLogUtil.d(ESSlidingSmoothTabView.class, "ScrollView宽度:" + mTabScrollView.getWidth());
		...
		if(mSelectedTabIndex<index && tabView.getRight()-scrollX > mTabScrollView.getWidth()){
			ESLogUtil.d(ESSlidingSmoothTabView.class, "右边被遮挡");
			int offsetX = 0;
			//右边被遮挡
			...
			ESLogUtil.d(ESSlidingSmoothTabView.class, "左边被遮挡");
			//左边被遮挡  offsetX是负值
			int offsetX = 0;
            if(index == 0){
            	offsetX = -scrollX;
