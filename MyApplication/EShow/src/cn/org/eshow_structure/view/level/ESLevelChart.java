 ...
 */
package cn.org.eshow_structure.view.level;
 ...
 */
public class ESLevelChart extends ESLevelAbstractChart {

	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = 1L;
	
	/** The m dataset. */
	protected ESLevelSeriesDataset mDataset;
	
	/** The m renderer. */
	protected ESLevelSeriesRenderer mRenderer;
	 ...
	 */
	public ESLevelChart(ESLevelSeriesDataset mDataset, ESLevelSeriesRenderer mRenderer) {
		super();
	 ...
	 */
	protected void setDatasetRenderer(ESLevelSeriesDataset dataset,ESLevelSeriesRenderer renderer) {
		mDataset = dataset;
		mRenderer = renderer;
	}
	 ...
	 * @see ESLevelAbstractChart#draw(android.graphics.Canvas, int, int, int, int, int, int, android.graphics.Paint)
	 */
	public void draw(Canvas canvas, int x, int y,int measureWidth,int measureHeight,int screenWidth, int screenHeight,
		...
		textLevelSize = ESViewUtil.scale(screenWidth, screenHeight, textLevelSize);
		//当前值文字与顶部的距离
        ...
        int textWidth = (int) ESGraphicUtil.getStringWidth(textValue, mTextPaint);
		...
		for(int i=0;i<color.length;i++){
			...
			if(textlevelIndex == i){
		        ...
		        int textDescWidth = (int) ESGraphicUtil.getStringWidth(textDesc, mTextPaint);
			...
			if(partValue!=null && partValue.length == color.length){
		        ...
		        int partValueWidth = (int) ESGraphicUtil.getStringWidth(String.valueOf(partValue[i]), mTextPaint);
