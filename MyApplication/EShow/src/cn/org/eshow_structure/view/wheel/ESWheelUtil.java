 ...
 */
package cn.org.eshow_structure.view.wheel;
...
import cn.org.eshow_structure.activity.ESActivity;
import cn.org.eshow_structure.util.ESDateUtil;
import cn.org.eshow_structure.util.ESDialogUtil;
import cn.org.eshow_structure.util.ESStrUtil;
import cn.org.eshow_structure.view.wheel.ESWheelView.AbOnWheelChangedListener;
...
public class ESWheelUtil {
	 ...
	 */
	public static void initWheelDatePicker(final ESActivity activity,final TextView mText,final ESWheelView mWheelViewY,final ESWheelView mWheelViewM,final ESWheelView mWheelViewD,
		...
		mText.setText(ESStrUtil.dateTimeFormat(defaultYear + "-" + defaultMonth + "-" + defaultDay));
		...
		okBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				ESDialogUtil.removeDialog(v.getContext());
				...
				mText.setText(ESStrUtil.dateTimeFormat(year + "-" + month + "-" + day));
			}
	 ...
	 */
	public static void initWheelTimePicker(final ESActivity activity,final TextView mText,final ESWheelView mWheelViewMD,final ESWheelView mWheelViewHH,final ESWheelView mWheelViewMM,
			 Button okBtn,Button cancelBtn,
		...
		String val = ESStrUtil.dateTimeFormat(defaultYear + "-" + defaultMonth + "-" + defaultDay + " " + defaultHour + ":" + defaultMinute + ":" + second) ;
		mText.setText(val);
		// 添加大小月月份并将其转换为list,方便之后的判断
		String[] months_big = { "1", "3", "5", "7", "8", "10", "12" };
		String[] months_little = { "4", "6", "9", "11" };
		...
		for(int i=1;i<13;i++){
			if(list_big.contains(String.valueOf(i))){
				...
				if(i==2){
					if(ESDateUtil.isLeapYear(defaultYear)){
						for(int j=1;j<28;j++){
							textDMList.add(i+"月"+" "+j+"日");
							textDMDateList.add(defaultYear+"-"+i+"-"+j);
						}
		...
		// 月日
		mWheelViewMD.setAdapter(new ESStringWheelAdapter(textDMList, ESStrUtil.strLength("12月" + " " + "12日")));
		...
		// 时
		mWheelViewHH.setAdapter(new ESNumericWheelAdapter(0, 23));
		...
		// 分
		mWheelViewMM.setAdapter(new ESNumericWheelAdapter(0, 59));
		...
		okBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				Calendar calendar = Calendar.getInstance();
				int second = calendar.get(Calendar.SECOND);
				String val = ESStrUtil.dateTimeFormat(dmStr + " " + index2 + ":" + index3 + ":" + second) ;
				mText.setText(val);
			}
	 ...
	 * @param initStart the init start
	 */
	public static void initWheelTimePicker2(final ESActivity activity,final TextView mText,final ESWheelView mWheelViewHH,final ESWheelView mWheelViewMM,
			 Button okBtn,Button cancelBtn,
		...
		String val = ESStrUtil.dateTimeFormat(defaultHour + ":" + defaultMinute) ;
		mText.setText(val);
		...
		okBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				int index2 = mWheelViewHH.getCurrentItem();
				int index3 = mWheelViewMM.getCurrentItem();
				String val = ESStrUtil.dateTimeFormat(index2 + ":" + index3) ;
				mText.setText(val);
			}
