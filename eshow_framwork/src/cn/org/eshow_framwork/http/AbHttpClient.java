...
import cn.org.eshow_framwork.cache.AbCacheHeaderParser;
import cn.org.eshow_framwork.cache.AbCacheResponse;
import cn.org.eshow_framwork.cache.AbCacheUtil;
import cn.org.eshow_framwork.cache.AbDiskBaseCache;
import cn.org.eshow_framwork.cache.AbDiskCache.Entry;
import cn.org.eshow_framwork.cache.http.AbHttpBaseCache;
import cn.org.eshow_framwork.global.AbAppConfig;
import cn.org.eshow_framwork.global.AbAppException;
import cn.org.eshow_framwork.http.entity.MultipartEntity;
import cn.org.eshow_framwork.http.ssl.EasySSLProtocolSocketFactory;
import cn.org.eshow_framwork.image.AbImageLoader;
import cn.org.eshow_framwork.task.thread.AbThreadFactory;
    			...
    			try {
    				...
    				if(entry == null || entry.isExpired()){
    					...
    					if(response!=null){
    						...
    						AbLogUtil.i(mContext, "1[HTTP GET]:" + httpUrl + ",result：" + responseBody);
    						...
    						if(entryNew!=null){
    							...
    							AbLogUtil.i(mContext, "HTTP 缓存失败，parseCacheHeaders失败");
    						}
	...
	private void doPost(String url,AbRequestParams params,AbHttpResponseListener responseListener){
		  try {
		      ...
		      if(isContainFile){
				  ...
				  AbLogUtil.i(mContext, "isContainFile is "+String.valueOf(isContainFile));
			  }
		      //取得HttpResponse
		      httpClient.execute(
                      httpPost,
    ...
    private class RedirectionResponseHandler implements ResponseHandler<String>{
        ...
        public String handleResponse(HttpResponse response) throws ClientProtocolException, IOException{
            ...
            if (statusCode == HttpStatus.SC_OK) {
                ...
                if (entity != null){
	  				  if(mResponseListener instanceof AbStringHttpResponseListener){
	                      ...
	                      String charset = EntityUtils.getContentCharSet(entity) == null ? encode : EntityUtils.getContentCharSet(entity);
	      	              ...
	      	              AbLogUtil.i(mContext, "2[HTTP GET]:" + request.getURI() + ",result：" + responseBody);
                          try {
                              ...
                              if(rr==-5){
                                  if("/user/login.json".equals(request.getURI().toString())){
                                      ((AbStringHttpResponseListener) mResponseListener).sendFailureMessage(statusCode, responseBody, new AbAppException(ret.getString("msg")));
                                  }else if("/user/check.json".equals(request.getURI().toString())) {
                                      ((AbStringHttpResponseListener) mResponseListener).sendFailureMessage(statusCode, responseBody, new AbAppException(AbAppConfig.ACCOUNT_ISEXIST));
                                  }
                              }else if(rr==-9){
                                  ((AbStringHttpResponseListener) mResponseListener).sendFailureMessage(statusCode, responseBody, new AbAppException(AbAppConfig.LOG_MISS_EXCEPTION));
                              }else if(rr==1){
                                  ((AbStringHttpResponseListener) mResponseListener).sendSuccessMessage(statusCode, responseBody);
                              }else {
                                  ((AbStringHttpResponseListener) mResponseListener).sendFailureMessage(statusCode, responseBody,null);
                              }
                          } catch (JSONException e) {
                              e.printStackTrace();
                              ((AbStringHttpResponseListener) mResponseListener).sendFailureMessage(statusCode, responseBody, e);
                          }
	  				  }else if(mResponseListener instanceof AbBinaryHttpResponseListener){
	  					  responseBody = "Binary";
	  					  AbLogUtil.i(mContext, "3[HTTP GET]:" + request.getURI() + ",result：" + responseBody);
	  					  ...
	  					  AbLogUtil.i(mContext, "4[HTTP GET]:" + request.getURI() + ",result：" + fileName);
                ...
                AbLogUtil.i(mContext,"other statusCode = "+statusCode);
            	//完成消息
                mResponseListener.sendFinishMessage();
