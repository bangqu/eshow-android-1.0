 ...
 */
package cn.org.eshow_framwork.image;
...
import cn.org.eshow_framwork.cache.image.ESBitmapResponse;
import cn.org.eshow_framwork.cache.image.ESImageBaseCache;
...
import cn.org.eshow_framwork.util.AbImageUtil;
...
public class AbImageLoader {
    ...
    public void download(final ImageView imageView,final String url,final int desiredWidth,final int desiredHeight,final OnImageListener onImageListener) { 
    	...
    	if(bitmap != null){
            ...
            item.setListener(new cn.org.eshow_framwork.task.AbTaskObjectListener(){
			...
			if(entry == null || entry.isExpired()){
				...
				if(response!=null){
					...
					if(bitmap!=null){
						...
						diskCache.put(cacheKey, cn.org.eshow_framwork.cache.AbCacheHeaderParser.parseCacheHeaders(response, cacheMaxAge));
					}
				}
			}else{
				//磁盘中有
